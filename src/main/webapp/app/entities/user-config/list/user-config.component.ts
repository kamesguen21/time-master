...
export class UserConfigComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['login', 'firstName', 'lastName', 'email', 'phoneNumber'];
  ...
  search(query: string): void {
    if (query && UserConfigComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  delete(userConfig: IUserConfig): void {
    const modalRef = this.modalService.open(UserConfigDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (UserConfigComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.userConfigs = this.refineData(dataFromBody);
  }

  protected refineData(data: IUserConfig[]): IUserConfig[] {
  ...
  protected queryBackend(predicate?: string, ascending?: boolean, currentSearch?: string): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.userConfigService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
